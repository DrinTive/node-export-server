...
const doExport = async (options, chartJson, endCallback, svg) => {
  ...
  const allowFileResourcesScoped =
    typeof customLogicOptions.allowFileResources === 'boolean'
      ? customLogicOptions.allowFileResources
      : allowFileResources;

  if (!customLogicOptions) {
    customLogicOptions = options.customLogic = {};
  } else if (allowCodeExecutionScoped && allowFileResourcesScoped) {
